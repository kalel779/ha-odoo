...
class OdooSalesCoordinator(DataUpdateCoordinator[OdooSalesData]):
    ...
    def __init__(
        ...
        self.activity_keyword = activity_keyword
    ...
    def _orders_to_process(self) -> int:
        ...
        If `activity_keyword` is set (integration options), only activities
        whose Summary contains that text are counted — e.g. only the
        "Nouvelle commande à traiter" To-Do created by that automation,
        ignoring any other activity a user might log on the order (calls,
        follow-ups, etc.). Left empty, ANY open activity counts.
        """
        domain = [("activity_ids", "!=", False), ("state", "!=", "cancel")]
        if self.activity_keyword:
            domain = [
                ("activity_ids.summary", "ilike", self.activity_keyword),
                ("state", "!=", "cancel"),
            ]
        return self.client.search_count("sale.order", domain)
