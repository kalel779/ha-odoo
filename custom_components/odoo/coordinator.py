...
class OdooSalesCoordinator(DataUpdateCoordinator[OdooSalesData]):
    ...
    def _orders_to_process(self) -> int:
        """Website orders that are confirmed but not yet locked/closed.

        Deliberately simple and compatible with every Odoo version (does not
        rely on a Stock-module-dependent field): orders in state 'sale'
        (confirmed) linked to the website.
        """
        domain = [("state", "=", "sale"), ("website_id", "!=", False)]
        return self.client.search_count("sale.order", domain)
